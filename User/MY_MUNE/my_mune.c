...
void Mune_Init(void)
{
	...
	
	{
		...
		Display = Create_Equal_NextMenu( Game,    "Display        ",MOD_Display);
		Sec_tick= Create_Equal_NextMenu( Display, "Sec_tick       ",MIAOBIAO);
		Apple   = Create_Equal_NextMenu( Sec_tick,"Apple          ",ACTION);
		Back 	= Create_Equal_NextMenu( Apple,   "Exit           ",ComeBack);
	}
	{
		//创建三级节点
		Test_1	= Create_NextMenu	   ( Tool,  "Edition        ",Edition_Funcation);
		Test_2	= Create_Equal_NextMenu( Test_1,"Time_Set       ",Time_Set_Funcation);
		Test_3	= Create_Equal_NextMenu( Test_2,"Sound_Set      ",SOUND);
//		Test_4	= Create_Equal_NextMenu( Test_3,"Mod_Display    ",ComeBack);
		Test_4	= Create_Equal_NextMenu( Test_3,"Exit           ",ComeBack);
	}
	{
		Game1 	= Create_NextMenu( Game		  ,"Game1          ",GAME1);
		Game2 	= Create_Equal_NextMenu( Game1,"Game2          ",GAME2);
